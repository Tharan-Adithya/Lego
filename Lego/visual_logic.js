 ...
 * Tue Apr 09 2024 18:37:23 GMT+0530 (India Standard Time)
...
var l, cubes, dictionary, x, history2, k, var2, j, pos, obj, obj1, o, loaded_data, z, m, y, n;
...
function drag(x) {
  objConstraintAddLimit(x, x, 'POS_Z', 7, 500, '', 0, 'LIMITDIST_ONSURFACE');
  registerOnDrag(x, false, [0,1,2], function() {}, function() {
    dragMove(x, 'XYZ', 'R7Ddss(6#)+pAS~RittM', 'g{nzw+|hR50oF?3O/?gz');
  }, function() {}, 'g{nzw+|hR50oF?3O/?gz');
  pos = getObjTransform(x, false, 'position', 'xyz');
  dictSet(dictionary, x, pos);
}
...
createPhysicsBody('STATIC', '6cube', 'MESH', 0);
createPhysicsBody('STATIC', '8cube', 'MESH', 0);
createPhysicsBody('STATIC', 'Cylinder.001', 'MESH', 0);

l = [];
for (k = 0; k <= 9; k++) {
  if (k == 0) {
    for (j = 2; j <= 9; j++) {
      obj = cloneObject('Cube.002');
      obj1 = cloneObject('Cube.001');
      if (j % 2 == 0) {
        setObjTransform(obj, false, 'position', [k * 20, j * 20, ''], false);
      } else {
        setObjTransform(obj1, false, 'position', [k * 20, j * 20, ''], false);
      }
      l.push(obj);
      l.push(obj1);
    }
...
changeVis('8cube', false);
changeVis('6cube', false);
changeVis('Cylinder.001', false);
o = 0;
z = 0;
m = 0;
addHTMLElement('button', 'button', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'red', 'button', false);
setHTMLElemStyle('height', '50px', 'button', false);
setHTMLElemStyle('width', '50px', 'button', false);
setHTMLElemStyle('alignItems', 'center', 'button', false);
eventHTMLElem('click', 'button', false, function(event) {
  x = cloneObject('Cylinder.001');
  o = (typeof o == 'number' ? o : 0) + 6;
  setObjTransform(x, false, 'position', [115, -75, o], false);
  changeVis('Cylinder.001', true);
  drag(x);
});
objConstraintAddLimit(x, 'Cylinder.001', 'POS_Z', 7, 500, '', 0, 'LIMITDIST_ONSURFACE');
registerOnDrag('Cylinder.001', false, [0,1,2], function() {}, function() {
  dragMove('Cylinder.001', 'XYZ', 'PWt$rb~@h2v,Z9EqPfid', 'u9Ay4n.{GREzS=_c{x0h');
}, function() {}, 'u9Ay4n.{GREzS=_c{x0h');
addHTMLElement('button', 'btn2', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'pink', 'btn2', false);
setHTMLElemStyle('height', '50px', 'btn2', false);
setHTMLElemStyle('width', '50px', 'btn2', false);
eventHTMLElem('click', 'btn2', false, function(event) {
  y = cloneObject('6cube');
  z = (typeof z == 'number' ? z : 0) + 6;
  setObjTransform(y, false, 'position', [80, -75, z], false);
  changeVis('6cube', true);
  drag(y);
});
objConstraintAddLimit(y, '6cube', 'POS_Z', 7, 500, '', 0, 'LIMITDIST_ONSURFACE');
registerOnDrag('6cube', false, [0,1,2], function() {}, function() {
  dragMove('6cube', 'XYZ', 'DRn1dfUE:?4*6Yo%UP]m', 'J.[hut#T6?lp)|(8*B[F');
}, function() {}, 'J.[hut#T6?lp)|(8*B[F');
addHTMLElement('button', 'btn3', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'green', 'btn3', false);
setHTMLElemStyle('height', '50px', 'btn3', false);
setHTMLElemStyle('width', '50px', 'btn3', false);
eventHTMLElem('click', 'btn3', false, function(event) {
  n = cloneObject('8cube');
  m = (typeof m == 'number' ? m : 0) + 5;
  setObjTransform(n, false, 'position', [100, -75, m], false);
  changeVis('8cube', true);
  drag(n);
});
objConstraintAddLimit(n, '8cube', 'POS_Z', 7, 500, '', 0, 'LIMITDIST_ONSURFACE');
registerOnDrag('8cube', false, [0,1,2], function() {}, function() {
  dragMove('8cube', 'XYZ', 'SU^4(h:0.Zqn3G86iDfv', 'Vhc1HxUk}ac2kvS6VeZ$');
}, function() {}, 'Vhc1HxUk}ac2kvS6VeZ$');

cubes = ['Cylinder.001', '8cube', '6cube'];

eventHTMLElem('keydown', x, false, function(event) {
  if (getEventProperty('code', event) == 'KeyW') {
    var2 = getObjTransform(x, false, 'position', 'z');
    setObjTransform(x, false, 'position', [0, 0, var2 + 7], false);
  }
});
...
addHTMLElement('button', 'save', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'yellow', 'save', false);
setHTMLElemStyle('height', '50px', 'save', false);
setHTMLElemStyle('width', '50px', 'save', false);
eventHTMLElem('click', 'save', false, function(event) {
  var VARS = Object.defineProperties({}, {
    "l": { get: function() { return l; }, set: function(val) { l = val; } },
    "cubes": { get: function() { return cubes; }, set: function(val) { cubes = val; } },
    "x": { get: function() { return x; }, set: function(val) { x = val; } },
    "dictionary": { get: function() { return dictionary; }, set: function(val) { dictionary = val; } },
    "history": { get: function() { return history2; }, set: function(val) { history2 = val; } },
    "k": { get: function() { return k; }, set: function(val) { k = val; } },
    "var": { get: function() { return var2; }, set: function(val) { var2 = val; } },
    ...
    "obj1": { get: function() { return obj1; }, set: function(val) { obj1 = val; } },
    "o": { get: function() { return o; }, set: function(val) { o = val; } },
    "z": { get: function() { return z; }, set: function(val) { z = val; } },
    ...
    "loaded_data": { get: function() { return loaded_data; }, set: function(val) { loaded_data = val; } },
});

  Function('app', 'v3d', 'VARS', 'PROC', (('VARS[\'history\']=JSON.stringify(VARS[\'dictionary\']);' + '\n' +
  'localStorage.setItem("vstorage",VARS[\'history\']);' + '\n' +
  'for (var key in dictionary) {' + '\n' +
  '    if (dictionary.hasOwnProperty(key)) {' + '\n' +
  '        var value = dictionary[key];' + '\n' +
  '        console.log("Key: " + key + ", Value: " + value);' + '\n' +
  '        // You can perform operations with the key and value here' + '\n' +
  '    }' + '\n' +
  '}')))(appInstance, v3d, VARS, PROC);

  LMSSetValue('cmi.suspend_data', history2);});

addHTMLElement('button', 'load', 'TO', ['CONTAINER'], false);
setHTMLElemStyle('backgroundColor', 'brown', 'load', false);
setHTMLElemStyle('height', '50px', 'load', false);
setHTMLElemStyle('width', '50px', 'load', false);
eventHTMLElem('click', 'load', false, function(event) {
  loaded_data = readJSON(LMSGetValue('cmi.suspend_data'));
  while (!(typeof loaded_data == 'object' ? !Object.keys(loaded_data).length : true)) {
  }
});
