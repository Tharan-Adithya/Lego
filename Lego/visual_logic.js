 ...
 * Wed Apr 17 2024 18:45:49 GMT+0530 (India Standard Time)
...
var l, cubes, dictionary, x, history2, k, y, var2, j, pos, obj, obj1, o, z, save, m, n, xx, newpiece;
...
eventHTMLElem('click', 'save', false, function(event) {
  downloadFile(history2, 'data.json');
});
...
eventHTMLElem('click', 'load', true, function(event) {
  openFile(function() {
    save = readJSON(_pGlob.openedFile);
    var n_list = typeof save == 'object' ? Object.keys(save) : [];
    for (var n_index in n_list) {
      n = n_list[n_index];
      if (textCount(n, 'Cylinder.001') == 1) {
        xx = 'Cylinder.001';
      } else if (textCount(n, '6cube') == 1) {
        xx = '6cube';
      } else {
        xx = '8cube';
      }
      newpiece = cloneObject(xx);
      setObjTransform(newpiece, false, 'position', [dictGet(save, n)[0], dictGet(save, n)[1], dictGet(save, n)[2]], false);
      changeVis(newpiece, true);
      drag(newpiece);
    }
  });
});
